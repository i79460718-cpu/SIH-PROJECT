    ...
    <div className="mx-auto max-w-[1360px] px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
        <div>
          ...
          <h1 className="display-font mt-2 text-3xl font-extrabold tracking-tight text-[hsl(var(--foreground))] sm:text-4xl">{t("map.headerTitle")}</h1>
          <p className="mt-1 text-sm text-[hsl(var(--muted-foreground))]">{t("map.headerDesc")}</p>
        </div>
        ...
        <div className="lg:col-span-8 min-h-[540px]">
          {isLoading ? (
            <div className="h-[540px] flex items-center justify-center rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--card))]">
              <LoadingBlock label={t("map.initEngine")} />
            </div>
          ) : isError ? (
            <div className="h-[540px] flex items-center justify-center rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--card))]">
              <ErrorBlock onRetry={() => refetch()} label={t("map.failLoad")} />
            </div>
          ) : (
          ...
          <div className="rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-5 shadow-sm">
            <div className="flex items-center justify-between border-b border-[hsl(var(--border))] pb-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-xs font-bold uppercase tracking-wider text-[hsl(var(--foreground))]">{t("map.activeClusters")}</h3>
              </div>
              <span className="mono-font text-[10px] text-[hsl(var(--muted-foreground))]">
                {filteredIssues.length} {t("map.activeIssues")}
              </span>
            </div>

            <div className="mt-3 divide-y divide-[hsl(var(--border)/.6)]">
              {filteredIssues.length === 0 ? (
                <p className="py-6 text-center text-xs text-[hsl(var(--muted-foreground))]">{t("map.noIssues")}</p>
              ) : (
                filteredIssues.map((issue) => (
                  <div key={issue.id} className="py-3 first:pt-1">
                    ...
                    <div className="mt-1.5 flex items-center justify-between text-[11px] text-[hsl(var(--muted-foreground))]">
                      ...
                      <span className="font-bold text-[hsl(var(--foreground))] shrink-0"> {issue.reportCount} {t("card.reportCount")} </span>
                    </div>
                    ...
                    <div className="mt-2 flex items-center justify-between">
                      ...
                      >
                        {t("card.trackIssue")} <ArrowRight size={11} />
                      </Link>
                    </div>
                  </div>
                ))

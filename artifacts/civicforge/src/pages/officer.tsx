  ...
  return (
    <div className="mx-auto max-w-[1040px] px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="rounded-2xl border-2 border-[hsl(var(--border))] bg-[hsl(var(--card))] p-6 shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="mono-font text-xs font-bold uppercase tracking-wider text-emerald-700 dark:text-emerald-300">{t("officer.portalName")}</span>
          ...
          <div className="sm:text-right">
            <label className="text-[10px] font-bold uppercase tracking-wider text-[hsl(var(--muted-foreground))] block mb-1">{t("officer.switchDemo")}</label>
            <select
      ...
      <div className="mt-4 rounded-xl border border-amber-500/40 bg-amber-500/10 p-4 text-xs text-amber-900 dark:text-amber-200 flex items-start gap-3">
        ...
        <div>
          <strong className="font-bold">{t("officer.triPartyRule")}</strong> {t("officer.triPartyDesc")}
        </div>
      </div>
      ...
      <div className="mt-8">
        <div className="flex items-center justify-between border-b border-[hsl(var(--border))] pb-3">
          <h2 className="text-base font-extrabold text-[hsl(var(--foreground))] flex items-center gap-2">
            <Wrench size={16} className="text-[hsl(var(--primary))]" />{t("officer.activeAssignments")} ({assignments?.length ?? 0})</h2>
          <span className="mono-font text-xs text-[hsl(var(--muted-foreground))]">{t("officer.assignedTo")} {currentOfficer?.name}</span>
        </div>

        {isAssignmentsLoading ? (
          <div className="mt-6">
            <LoadingBlock label={t("officer.fetchingTasks")} />
          </div>
        ) : assignments?.length === 0 ? (
          <div className="mt-6 rounded-2xl border border-dashed border-[hsl(var(--border))] p-8 text-center text-xs text-[hsl(var(--muted-foreground))]">{t("officer.noTasks")}</div>
        ) : (
                  ...
                  <div className="mt-3">
                    ...
                    <div className="mt-3 flex flex-wrap items-center gap-4 text-xs text-[hsl(var(--muted-foreground))]">
                      ...
                      <span className="flex items-center gap-1 font-bold text-[hsl(var(--foreground))]">
                        ...
                        {task.reportCount} {t("issue.addReport")}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-[hsl(var(--border)/.6)] pt-3.5">
                    <div className="flex flex-wrap items-center gap-2">
                      {task.status === "Officer Assigned" && (
                        ...
                        >
                          <CheckCircle2 size={13} />{t("officer.acceptTask")}</button>
                      )}
                      ...
                      {task.status === "Work Started" && (
                        ...
                        >
                          <Camera size={13} />{t("officer.uploadEvidenceBtn")}</button>
                      )}

                      {isAwaitingVerification && (
                        <span className="mono-font rounded bg-purple-500/20 px-2.5 py-1 text-xs font-bold text-purple-800 dark:text-purple-300">{t("officer.awaitingCitizen")}</span>
                      )}

                      {isResolved && (
                        <span className="mono-font rounded bg-emerald-500/20 px-2.5 py-1 text-xs font-bold text-emerald-800 dark:text-emerald-300">{t("officer.resolvedClosed")}</span>
                      )}
                    </div>
      ...
      {evidenceModalIssue && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
          <div className="w-full max-w-lg rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-6 shadow-2xl">
            <div className="flex items-center justify-between border-b border-[hsl(var(--border))] pb-3">
              <h3 className="text-base font-bold text-[hsl(var(--foreground))]">{t("officer.uploadEvidenceTitle")}</h3>
              <button
                type="button"
            ...
            <p className="mt-2 text-xs text-[hsl(var(--muted-foreground))]">
              {t("officer.attachProof")} <strong>{evidenceModalIssue.publicId}</strong> {t("officer.forVerification")}
            </p>

            <div className="mt-4 space-y-4">
              <div>
                <label className="text-[11px] font-bold uppercase text-[hsl(var(--muted-foreground))]">{t("officer.photoUrl")}</label>
                <input
                  type="text"
              ...
              <div>
                <label className="text-[11px] font-bold uppercase text-[hsl(var(--muted-foreground))]">{t("officer.notes")}</label>
                <textarea
                  rows={2}
            ...
            <div className="mt-6 flex justify-end gap-2">
              ...
              >
                {addEvidenceMutation.isPending ? t("action.loading") : t("officer.submitProof")}
              </button>
            </div>
          </div>
        </div>
